...
#[structopt(name = "Blue grouse migrator")]
enum Opt {
...
async fn main() {
    ...
    match opt {
        ...
        Opt::Add { name } => add_migration_file(&name),
        Opt::Run => run_migrations().await, //println!("Running all the files!"),
    }
...
async fn create_migration_table(pool: &mut PgPool) {
    sqlx::query(
        r#"
